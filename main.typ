...
Everything in this paper can be reproduced from the companion repository @studexample2026, which holds the shared analysis module, the numbered scripts that build the meshes, run the eigenbuckling analyses, compute the reference values, and draw the figures, and the resolved Julia environment. Two entry points are meant for readers rather than for reproduction. A Pluto.jl notebook @pluto2026 opens in the web browser and walks through the example cell by cell: the section polyline, the reference values, the mesh around a hole, an optional live solve of the lowest modes (about one minute on a laptop), and an interactive three-dimensional viewer of the characteristic modes with an amplitude slider. A standalone HTML page exported with Bonito and WGLMakie @danisch2021makie shows the local, hole, distortional, and global modes of the model as deformed shapes with orbit and zoom controls and needs no Julia installation; it is served from the repository's GitHub Pages site.
